###
"""
This is a test without sensor signal. Only saving images from depth and/or mv cams.
"""
...
class depth_cam:
    def __init__(self):
        ...
        # self.camRgb.setResolution(dai.ColorCameraProperties.SensorResolution.THE_2024X1520) # 4056x3040
...
# def setup_and_start_stream(device, node, width, height, pixel_format, exposure_auto, num_buffers): 
#     setup(device, node, width, height, pixel_format, exposure_auto)
#     device.start_stream(num_buffers)

## Original values [4096, 3000, 'BayerRG8', 27362.104, 'Continuous', 1500]
...
class MVCam:
    def __init__(self):
        ...
        self.nodes, self.values = store_initial_nodes(self.device)
        logger.info(f"self.values")
        # setup(self.device, self.nodes, 2700, 2000, 'Mono8', 'Off')
    ...
    def capture(self):
        try:
            ...
            # cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/mv_rgb/rgb_frame_{currentDateAndTime}_{vseq_no}.jpg", rgb_img)
            cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/mv_rgb/rgbFrame_{currentDateAndTime}.jpg", rgb_img)
            ...
            try:
                ...
                while True:
                    ...
                    if not is_duplicated(vseq_no):
                        ...
                        rgb_img=mv_cam.capture()
                        image_path = onvif_cam.get_snapshot_and_save(now_strf)

                        qdata_depth = [str_data, depth_camera.messages]
                        ...
                        sampyo_processes = [
                            Process(target=main_ptz, args=(queue_ptz,)),
                            Process(target=main_mv, args=(queue_mv,)),                            
                    ...
                    #                 cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_disparity/disparity_frame_{disparity_time}_{item_code}_{type_code}_{from_code}.jpg", disparity_frame)
                    ...
                    #                 # cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_rgb/rgb_frame_{currentDateAndTime}_{vseq_no}.jpg", rgb_frame)
                    #                 # cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_rgb/rgb_frame_{datetime.datetime.now()}_{vseq_no}.jpg", rgb_frame)
                    #                 cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_rgb/rgb_frame_{current_time_depth_rgb}_{item_code}_{type_code}_{from_code}.jpg", rgb_frame)                                    
...
#################################
    ...
    #             depth_camera.messages = depth_camera.device.getOutputQueue("rgbd").tryGetAll()    
    #             # while True:
    #             start = time.time() 
...
# #########################################
# # MV & Depth cam together without sensor#
# #########################################
#     with depth_camera.device:
#         depth_camera.device.startPipeline(depth_camera.pipeline)
#         while True:
...
#                             # cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_depth/depthFrame_{currentDateAndTime}_{vseq_no}.jpg", depth_frame)
#                             cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_depth/depthFrame_{currentDateAndTime}.jpg", depth_frame)
#                             logger.info(f"depth_frame collected: {depth_frame.shape}")
#                             logger.info(f"depth_frame depth:  {depth_frame}")
#                             print(depth_frame)
#                             print('depth iamge collected')
#                             with open(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_depth_array/depthFrameArray_{currentDateAndTime}.pickle", 'wb') as f:
#                                 pickle.dump(depth_frame, f)
...
#                             # cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_rgb/rgb_frame_{currentDateAndTime}_{vseq_no}.jpg", rgb_frame)
#                             cv2.imwrite(f"/home/sdt/Workspace/onvif/python-onvif-zeep/hojun/image_test/depth_rgb/rgb_frame_{currentDateAndTime}.jpg", rgb_frame)
#                             logger.info(f"depth_rgb_frame collected: {rgb_frame.shape}")
#                             # print('RGB: ', rgb_frame)
#                             print('RGB iamge collected')
#                             print('RGB: ', rgb_frame.shape)
...
#                 rgb_frame * depth_frame
    ...
    # while True:
        ...
        #rgb_img=mv_cam.capture() ##이것이 이미지를 캡쳐하는 함수이다 
        #logger.info(rgb_img) 
        #logger.info('mv image saved') 
        # depth_camera.capture() ### depth image와 그와 얼라인하는 RGB 이미지 저장하는 메서드
        # logger.info('depth image saved')  
        ...
        #             depth_camera.capture() ### depth image와 그와 얼라인하는 RGB 이미지 저장하는 메서드
                    ...
                    ## hojuns code come in here.

                    ## capture rgb, disparity, and depth
                    ## segment only the cargo truck
                    ...
                    # logger.info(f"Trigger type: {str_data.split('|')[-1][0]}")
                    #try:
                    #    if str_data.split('|')[-1][0] == "L":
                    #        time.sleep(4)
                    #except Exception as e:
                    #    logger.error(traceback.format_exc())
                    
        #             mv_cam.capture(vseq_no) ##이것이 이미지를 캡쳐하는 함수이다 
        #             depth_camera.capture(vseq_no) ### depth image와 그와 얼라인하는 RGB 이미지 저장하는 메서드
