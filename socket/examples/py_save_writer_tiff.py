...
Save: Tiff
   ...
   shows the construction of image writer, and saves a single TIFF image with 
...
def save(buffer):
	'''
	demonstrates saving a TIFF image
	...
	Prepare image writer
		When saving as .tiff file, the writer optionally can take compression, 
		and cmykTags as configuration parameters of image(s) it would save. 
		If these arguments are not passed at run time, the Writer.save() 
		function will configure the writer to defualt compression and cmykTags.
	'''
	print(f'{TAB1}Prepare Image Writer')
	writer = Writer()
	writer.pattern = 'images/py_save_writer_tiff/image_<count>.tiff'
	''' 
	Save function for .tiff file
		buffer :
			buffer to save.
		kwargs (optional args) ignored if not applicable to an .tiff image:
			- 'compression', default is SC_NO_TIFF_COMPRESSION.
				Compression algorithm
			- 'cmykTags', default is False.
				If true, stores tags for separated CMYK
        		Otherwise, does not
	'''
	writer.save(converted, compression=_enums.SC_TIFF_COMPRESSION_LIST.SC_NO_TIFF_COMPRESSION, cmykTags=False)
