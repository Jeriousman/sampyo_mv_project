...
class MVCam:
    """
    에어리어 스캔 카메라를 초기화하고 필요한 설정값을 설정한다.
    """
    def __init__(self):
        try:
...
class granule_sam:
    """
    Segment anything 2 모델을 사용하여 object segmentation을 진행한다.
    그 후 segmented된 입자의 갯수를 파악하고, 그 입자들의 크기를 잰다.
    그 입자들의 픽셀 갯수를 세었으면, 실제로 계산한 실제거리를 픽셀 갯수와 결부한다. 
    마지막으로 입자 크기를 거름채로 삼표에서 거르는 로직에 입각하여 비슷한 논리로 입자들의 크기를 분류한다.
    """
    def __init__(self):
        ...
        self.model_cfg = "configs/sam2.1/sam2.1_hiera_l.yaml" ## SAM2의 업데이트된 2.1 버전 l (large) 모델 사용. Large 모델이 성능이 가장 뛰어나나, resource heavy.
    ...
    def forward_granule(self, img):
        ...
        # mask generator load
        if self.mixed:
            ## mixed-precison을 사용하면, 현재 헤비한 컴퓨팅을 최소한의 성능 저하를 통해 줄일 수 있다.
        ...
        pix_mm = 0.316452   ##0.316452=1 픽셀을 실제자로재었을때의 값 

        ## 13mm, 20mm, 25mm, 40mm 4가지 입자 크기 카테고리로 분류
        ...
        largerthan_40 = [] 
        ############################################################################################################################################
        ## 검출된 각 입자들의 크기를 재고 분류 논리에 따라 분류한 후 로그를 남기고 DB에 저장 및 이미지 저장
        ...
        ## 확인용으로 25~40mm 입자 크기 분류 카테고리로 들어가는 입자들을 나타내는 이미지 저장
        plt.axis("off")
        plt.imshow(overlay)
        ...
        granule_analysis_dict = {f"Smaller than 13mm: {len(lessthan_13)}, 13~20mm: {len(between_13_20)}, 20~25mm: {len(between_20_25)}, 25~40mm: {len(between_25_40)}, Bigger than 40mm: {len(largerthan_40)}"}
        return result_image, granule_analysis_dict

        ##다연님이 보내준 겹치지 않게 확인하는 방법 
    ...
    def analyze_granule(self, image):
        """
        이전에 사용하였으나, 현재는 forward_granule에서 모두 맡아서 진행하고 있으므로 사용하지 않는 함수.
        """
        try:
            mm13 = 0
            mm20 = 0
...
def main_ptz(q, model, model_item, sampyo_msdb, sdt_msdb):
    """
    PTZ카메라 queue에 이미지를 저장된 데이터를 빼온다. 그 안에는 이미지에 대한 메타데이터와 이미지가 있다.
    데이터를 통해 원재료 분류 등 인퍼런스를 실행하고 DB에 저장한다.  
    """
    try:
        packets = q.get()
...
def main_mv(q, granule_model, sdt_msdb):
    """
    에어리어 스캔 카메라 queue에서 데이터를 불러온 후,
    모래가 아닌 자갈 입자들만 있을 때 입도분석을 한다.
    그 후 DB에 저장 
    """
    try:
        packets = q.get()
        ...
        if item_code in ["31013", "31019", "31025", "31067"]:  ##"32001" -> 모래   
            ...
            while True:
                ...
                #    logger.info("sam starts running")
                    ...
                    sdt_msdb.upsert_mv(str_data, granule_data)
...
def main_depth(q, sdt_msdb):
    """
    뎁스카메라 queue에서 데이터를 불러온 후,
    체적분류를 하기 위한 다양한 태스크를 진행한다.
    그 후 체적분류 결과를 DB에 저장한다. 
    """
    try:
        packets = q.get()
        ...
        """
        MaskDINO 모델을 사용하여 화물차의 컨테이너 안쪽만 참고할 수 있도록 컨테이너 영역 마스킹을 인퍼런스 
        """
        while True:
            try:
        ...
        """
        뎁스카메라를 사용해서 나온 결과물로 높이 측정하는 로직.
        """
        camera_ground_depth = np.where((depth_frame >= 9894) & (depth_frame <= 13192), depth_frame, 0) ##경험적으로 9894 ~ 13192 mm높이가 바닥을 나타내는 픽셀인것읗 확인
        ##len(camera_ground_depth)
        camera_ground_depth_nonezero = camera_ground_depth[camera_ground_depth != 0] ##numpy array에서 값이 0인 것을 제외하여 정확성 up.
        ##len(camera_ground_depth_nonezero)
        average_ground_depth = camera_ground_depth_nonezero.mean()  ##컨테이너안을 나타내는 array의 모든 픽셀들을 평균을내어, 컨테이너안의 평균 높이를 측정하고, 이를 컨테이너 높이로 간주한다.
        logger.info(f"Average ground depth: {average_ground_depth}")

        ## 화물칸 뎁스  6501~8400. 경험적으로 컨테이너 안의 높이들은 6501~8400mm의 값을 가지는 것을 확인하여 6501~8400 값으로 필터링 하였다. 
        lower_boundary = 6501  
        upper_boundary = 8400
...
if __name__ == "__main__":
    ...
    with depth_camera.device:
        ...
        ##이전세션에서 남아있는 찌꺼기 데이터가 있으면 뱉어버리기 위한 for loop 
        for _ in range(20):
            ...
            try:
                ...
                while True:
                    ...
                    if not is_duplicated(vseq_no):
                        ...
                        ##Python multi-processing을 사용하여 3가지 카메라 동작 
                        sampyo_processes = [
